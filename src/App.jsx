...
function App() {
  ...
  const visibleMenu = menu.filter(
    ([key]) => isAdmin || ["pos", "stock", "sales", "cash"].includes(key),
  );
    ...
    <div className="app-shell">
      ...
      <main className="main">
        ...
        <div className="content">
          ...
          {page === "products" && (
            <Products products={products} setProducts={updateProducts} />
          )}
    ...
    <div className="login-page">
      <div className="login-card">
        <div className="brand-logo">
          <img src="/images/logo-pos.webp" alt="Logo del negocio" />
        </div>
        <h1>POS Central</h1>
  ...
  return (
    <aside className="sidebar">
      ...
      <div className="location-badge">
        <Building2 size={15} />{" "}
        {user.locationId ? getLocationName(user.locationId) : "Administración"}
      </div>
      <nav>
...
function Dashboard({ products, sales, movements }) {
  const totalToday = sales
    .filter((s) => s.date.startsWith("2026-08-14"))
    .reduce((a, b) => a + b.total, 0);
  const low = products
    .flatMap((p) =>
      locations.map((l) => ({ ...p, location: l, qty: p.stock[l.id] })),
    )
    ...
    <>
      <div className="page-intro">
        <div>
          ...
          <p>Vista consolidada de los 3 locales.</p>
        </div>
      ...
      <div className="dashboard-grid">
        <Panel title="Ventas por local" action="Hoy">
          {locations.map((l) => {
            const total = sales
              .filter(
                (s) => s.locationId === l.id && s.date.startsWith("2026-08-14"),
              )
              .reduce((a, b) => a + b.total, 0);
            const max = Math.max(
              ...locations.map((x) =>
                sales
                  .filter(
                    (s) =>
                      s.locationId === x.id && s.date.startsWith("2026-08-14"),
                  )
                  .reduce((a, b) => a + b.total, 0),
              ),
              1,
          ...
          {low.slice(0, 6).map((x, i) => (
            <div className="alert-row" key={i}>
              ...
              <div>
                <b>{x.name}</b>
                <span>
                  {x.location.name} · mínimo {x.minStock}
                </span>
              </div>
              <strong>{x.qty}</strong>
            </div>
    ...
    <>
      <div className="page-intro">
        <div>
          ...
          <p>Precios, costos y parámetros de reposición.</p>
        </div>
        ...
        >
          {filtered.map((p) => (
            <tr key={p.id}>
              ...
              <td>
                <Badge text={p.active ? "Activo" : "Inactivo"} />
              </td>
              <td>
                <button
      ...
      {modal && (
        <ProductModal
          ...
          onSave={(data) => {
            setProducts((prev) => [
              ...prev,
              { ...data, id: Date.now(), stock: { 1: 0, 2: 0, 3: 0 } },
            ]);
            setModal(false);
          }}
        />
    ...
    <Modal title="Nuevo producto" onClose={onClose}>
      <div className="form-grid">
        ...
        <Field
          ...
          value={data.barcode}
          onChange={(v) => setData({ ...data, barcode: v })}
        />
        <Field
          label="Costo"
          type="number"
          value={data.cost}
          onChange={(v) => setData({ ...data, cost: Number(v) })}
        />
        <Field
          label="Precio de venta"
          type="number"
          value={data.price}
          onChange={(v) => setData({ ...data, price: Number(v) })}
        />
        <label>
          Categoría
          <select
        ...
        <Field
          ...
          type="number"
          value={data.minStock}
          onChange={(v) => setData({ ...data, minStock: Number(v) })}
        />
      </div>
      <div className="modal-actions">
        ...
        <button className="primary" onClick={() => onSave(data)}>
          Guardar producto
        </button>
      </div>
    </Modal>
...
function Stock({ products }) {
  ...
  return (
    <>
      <div className="page-intro">
        <div>
          <h3>Stock por local</h3>
          <p>Control consolidado y alertas de reposición.</p>
        </div>
      ...
      <Panel title="Inventario actual">
        ...
        >
          {products.map((p) => {
            ...
            const shown = location === "all" ? true : Number(location);
            return (
              <tr key={p.id}>
                <td>
                  <b>{p.name}</b>
                ...
                {locations.map((l) => (
                  <td
                    key={l.id}
                    className={
                      p.stock[l.id] <= p.minStock ? "stock-low" : "stock-ok"
                    }
                  >
                    {shown === true || shown === l.id ? p.stock[l.id] : "—"}
                  </td>
                ))}
                <td>
                  <b>{total}</b>
                ...
                <td>
                  {locations.some((l) => p.stock[l.id] <= p.minStock) ? (
                    <Badge text="Reponer" danger />
                  ) : (
                    <Badge text="OK" />
                  )}
                </td>
...
function POS({ user, products, setProducts, setSales, setMovements }) {
  ...
  const add = (p) =>
    setCart((prev) => {
      ...
      if (existing)
        return prev.map((i) =>
          i.id === p.id
            ? { ...i, qty: Math.min(i.qty + 1, p.stock[locationId]) }
            : i,
        );
      return [...prev, { ...p, qty: 1 }];
    });
  ...
  const checkout = () => {
      ...
      prev.map((p) => {
        ...
        return item
          ? {
              ...
              stock: {
                ...p.stock,
                [locationId]: p.stock[locationId] - item.qty,
              },
            }
          : p;
      }),
    ...
    setMovements((prev) => [
      {
        id: Date.now(),
        date: "14/08/2026 14:00",
        type: "Venta",
        product: cart.map((x) => x.name).join(", "),
        location: getLocationName(locationId),
        quantity: -cart.reduce((a, b) => a + b.qty, 0),
        user: user.name,
      },
      ...prev,
    ]);
    setCart([]);
    setNotice(`Venta #${number} registrada correctamente.`);
    ...
    <>
      <div className="pos-layout">
        <section className="pos-products panel">
          <div className="panel-head">
            <h3>Productos</h3>
            <span className="pos-local">
              <Building2 size={15} /> {getLocationName(locationId)}
            </span>
          </div>
          ...
          <div className="product-grid">
            {available.map((p) => (
              ...
              >
                ...
                <small>Stock: {p.stock[locationId]}</small>
              </button>
            ))}
          </div>
        </section>
        <section className="cart panel">
          ...
          {cart.length === 0 ? (
            <div className="empty-cart">
              ...
              <b>Carrito vacío</b>
              <span>Seleccioná productos para comenzar.</span>
            </div>
          ) : (
          ...
          <div className="cart-bottom">
            ...
            <button
              ...
              onClick={checkout}
            >
              <CheckCircle2 size={18} /> Cobrar {formatMoney(total)}
            </button>
            {notice && (
...
function Purchases({ products, setProducts, setMovements }) {
  const [productId, setProductId] = useState(products[0].id),
    [qty, setQty] = useState(10),
    [localId, setLocalId] = useState(1),
    ...
    setProducts((prev) =>
      prev.map((x) =>
        x.id === p.id
          ? {
              ...x,
              stock: { ...x.stock, [localId]: x.stock[localId] + Number(qty) },
            }
          : x,
      ),
    );
    setMovements((prev) => [
      {
        ...
        type: "Compra",
        product: p.name,
        location: getLocationName(localId),
        quantity: Number(qty),
        user: "María Administradora",
      },
      ...prev,
    ]);
    setSaved(true);
  ...
  return (
    <>
      <div className="page-intro">
        <div>
          <h3>Compras</h3>
          <p>Ingreso de mercadería y reposición de locales.</p>
        </div>
      ...
      <Panel title="Flujo recomendado">
        <div className="steps">
          <Step
            ...
            text="Cargar proveedor, productos y cantidades."
          />
          <Step
            n="2"
            title="Distribuir"
            text="Asignar mercadería a cada local."
          />
          <Step
            n="3"
            title="Controlar"
            text="Verificar stock y alertas de reposición."
          />
        </div>
      </Panel>
    </>
...
function Transfers({ products, setProducts, setMovements }) {
  const [productId, setProductId] = useState(products[0].id),
    [qty, setQty] = useState(5),
    [from, setFrom] = useState(1),
    [to, setTo] = useState(2),
    [msg, setMsg] = useState("");
  const transfer = () => {
    if (from === to) return setMsg("El origen y destino deben ser diferentes.");
    const p = products.find((x) => x.id === Number(productId));
    if (p.stock[from] < qty)
      return setMsg("No hay stock suficiente en el local de origen.");
    setProducts((prev) =>
      prev.map((x) =>
        x.id === p.id
          ? {
              ...
              stock: {
                ...x.stock,
                [from]: x.stock[from] - Number(qty),
                [to]: x.stock[to] + Number(qty),
              },
            }
          : x,
      ),
    ...
    setMovements((prev) => [
      {
        ...
        type: "Transferencia",
        product: p.name,
        location: `${getLocationName(from)} → ${getLocationName(to)}`,
        quantity: Number(qty),
        user: "María Administradora",
      },
      ...prev,
    ]);
    setMsg("Transferencia registrada correctamente.");
  };
  return (
    <>
      <div className="page-intro">
        <div>
          ...
          <p>Movimientos de mercadería entre locales.</p>
        </div>
      </div>
      <Panel title="Nueva transferencia">
        <div className="transfer-flow">
          ...
          <ArrowRightLeft className="transfer-arrow" />
          <div className="flow-box">
            <span>Destino</span>
        ...
        <button className="primary" onClick={transfer}>
          Confirmar transferencia
        </button>
        {msg && (
          <div
            className={
              msg.includes("correctamente") ? "success-box" : "error-box"
            }
          >
            {msg}
          </div>
  ...
  return (
    <>
      <div className="page-intro">
        <div>
          <h3>Ventas</h3>
          <p>Historial de operaciones realizadas.</p>
        </div>
        <button className="secondary">
      ...
      <Panel title="Historial de ventas">
        ...
        >
          {sales.map((s) => (
            <tr key={s.id}>
              <td className="mono">X-{s.number}</td>
              <td>{s.date}</td>
              <td>{getLocationName(s.locationId)}</td>
              <td>{s.seller}</td>
              <td>{s.items}</td>
              <td>
                <Badge text={s.payment} />
              </td>
              <td>
  ...
  return (
    <>
      <div className="page-intro">
        <div>
          <h3>Caja</h3>
          <p>Apertura, movimientos y cierre por local.</p>
        </div>
      ...
      <div className="stats-grid">
        ...
        <Stat
          ...
          icon={CircleDollarSign}
          trend="Todas las cajas"
        />
        <Stat
          title="Efectivo esperado"
          value={formatMoney(50000 + total)}
          icon={Calculator}
          trend="Demo"
        />
      </div>
      <Panel title="Estado de cajas">
        <Table
          headers={["Local", "Estado", "Apertura", "Ventas", "Último cierre"]}
        >
          {locations.map((l) => (
            <tr key={l.id}>
              <td>
  ...
  return (
    <>
      ...
      <Panel title="Usuarios">
        <Table headers={["Usuario", "Rol", "Local", "Estado", "Permisos"]}>
          {demoUsers.map((u) => (
            <tr key={u.id}>
              <td>
                <b>{u.name}</b>
              ...
              <td>{u.locationId ? getLocationName(u.locationId) : "Todos"}</td>
              <td>
                <Badge text="Activo" />
              </td>
              <td>
                <button className="link-button">Ver permisos</button>
              </td>
            </tr>
          ))}
        </Table>
      </Panel>
      <Panel title="Matriz de permisos">
        <div className="permission-grid">
          {[
            ["Administrador", "Acceso total"],
            ["Encargado", "Ventas · Stock · Caja · Reportes"],
            ["Vendedor", "Ventas · Stock de su local · Caja"],
          ].map((x) => (
            <div className="permission-card" key={x[0]}>
              <ShieldCheck size={18} />
              <b>{x[0]}</b>
              <span>{x[1]}</span>
            </div>
  ...
  return (
    <>
      <div className="page-intro">
        <div>
          <h3>Reportes</h3>
          <p>Indicadores básicos para la administración.</p>
        </div>
        <select>
      ...
      <div className="stats-grid">
        <Stat
          title="Ventas acumuladas demo"
          value={formatMoney(sales.reduce((a, b) => a + b.total, 0))}
          icon={CircleDollarSign}
          trend="Período"
        />
        <Stat
          ...
          value={formatMoney(
            sales.reduce((a, b) => a + b.total, 0) / sales.length,
          )}
          icon={Calculator}
          trend="Por venta"
        />
        <Stat
          title="Margen unitario promedio"
          value={formatMoney(
            products.reduce((a, b) => a + (b.price - b.cost), 0) /
              products.length,
          )}
          icon={BarChart3}
          trend="Productos"
      ...
      <Panel title="Stock total por producto">
        <Table
          headers={[
            "Producto",
            "Stock total",
  ...
  return (
    <>
      <div className="page-intro">
        <div>
          <h3>Auditoría</h3>
          <p>Registro de movimientos sensibles y operaciones.</p>
        </div>
        <button className="secondary">
...
function Badge({ text, danger }) {
  return (
    <span className={"badge " + (danger ? "badge-danger" : "")}>{text}</span>
  );
}
